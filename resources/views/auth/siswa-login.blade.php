...
<div class="right-content-3-5">
    ...
    <h3 class="title-text-content-3-5">{{ __('Login Siswa') }}</h3>
    <p class="caption-text-content-3-5">Please log in using that account has<br>
        registered on the website.</p>
    <form style="margin-top: 1.5rem;" action="{{ url('login/siswa/proses') }}" method="post">
        @csrf
        <div style="margin-bottom: 1.75rem;">
            <label for="" class="d-block input-label-content-3-5">{{ __('Nomor Induk Siswa Nasional') }}</label>
            <div class="d-flex w-100 div-input-content-3-5">
                ...
                <input class="input-field-content-3-5 @error('nisn') is-invalid @enderror" type="number" name="nisn" value="{{ old('nisn') }}" placeholder="Masukkan NISN Anda" autofocus required>
                @error('nisn')
        ...
        <div style="margin-top: 1rem;">
            <label for="" class="d-block input-label-content-3-5">{{ __('Nama Lengkap') }}</label>
            <div class="d-flex w-100 div-input-content-3-5">
                <svg class="icon-content-3-5" style="margin-right: 1rem;" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <input class="input-field-content-3-5 @error('nama_siswa') is-invalid @enderror" type="text" name="nama_siswa" value="{{ old('nama_siswa') }}" placeholder="Masukkan Nama Siswa" required>
                @error('nama_siswa')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <button class="btn btn-fill-content-3-5 d-block w-100" type="submit">{{ __('Login') }}</button>
    </form>
    <p class="text-center bottom-caption-content-3-5">Untuk Akses Petugas Silahkan disini.
        <span class="green-bottom-caption-content-3-5"><a href="{{ url('login') }}">Klik Disini</a></span>
    </p>
</div>
@endsection